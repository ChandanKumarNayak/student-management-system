...
<title>Student Login | BCVM</title>
</head>
<body>
<div class="limiter">
			...
			<div class="wrap-login100 p-l-85 p-r-85 p-t-55 p-b-55">
				<form class="login100-form validate-form flex-sb flex-w" method="post" action="">
					...
					<div class="wrap-input100 validate-input m-b-36" data-validate = "Username is required">
						<input class="input100" type="text" name="student_username" placeholder="Enter username" required>
...
if(isset($_POST['login'])){
  ...
  $sqluser="SELECT password FROM student WHERE password='$password' ";
  $qresult=mysqli_query($db, $sqluser);
  $count=mysqli_num_rows($qresult);
  if($count == 0)
  {echo '<script language="javascript">alert("You entered a wrong password.");</script>';}
	  
  $query = "SELECT * FROM student WHERE student_username = '$student_username' AND password = '$password' AND status = '1'";
...
{
$query1 = "SELECT * FROM student WHERE student_username = '$student_username' AND password = '$password' AND status = '0'";
  $results1 = mysqli_query($db,$query1);
  if(mysqli_num_rows($results1) == 1){
                $_SESSION['student_username'] = $student_username;
             
  echo '<meta http-equiv="refresh" content="0;student_details.php" />';
  }
  else
  {
