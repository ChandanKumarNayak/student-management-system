...
<div class="w3-row">
  ...
  <center><h3 style="font-size:25px;color:red;width:100%;font-family:Arial"><b>-: USERNAME :-</b></h3></center>
    ...
    <input type="text" class="input" name="student_username1" placeholder="Enter current username" required>
    <div class="line-box">
      <div class="line"></div>
    </div>
  </label>
  <label>
	...
	<input type="text" class="input" name="student_username2" placeholder="Enter new username" required>
    <div class="line-box">
      <div class="line"></div>
    </div>
  </label>
  <label>
	...
	<input type="password" class="input" name="student_username3" placeholder="Confirm new username" required>
    <div class="line-box">
      <div class="line"></div>
    </div>
  </label>
 ...
 if(isset($_POST['change_username']))
{
 ...
 $student_username1 = $_POST['student_username1'];

  $sqluser="SELECT student_username FROM student WHERE student_username='$student_username1' ";
...
{echo '<script language="javascript">alert("Current username is wrong.");</script>';}
 ...
 $student_username2 = $_POST['student_username2'];

  $sqluser="SELECT student_username FROM student WHERE student_username='$student_username2' ";
  $qresult=mysqli_query($db, $sqluser);
  $count=mysqli_num_rows($qresult);
  if($count > 0)
  {
  ...
  {
  $student_username3 = $_POST['student_username3'];

 
  if (empty($student_username1)) { array_push($errors, "Current username is required"); }
  if (empty($student_username2)) { array_push($errors, "New username is required"); }
  if (empty($student_username3)) { array_push($errors, "Confirm username is required"); }
  if ($student_username2 != $student_username3) 
   {echo '<script language="javascript">alert("New username & confirm new username did not match.");</script>';}
   else
   if(count($errors) == 0){

$sql="UPDATE `student` SET student_username='$student_username2' WHERE student_username='".$_SESSION['student_username']."' AND student_username = '$student_username1'";
if($result = mysqli_query($db, $sql))
if(($result) > 0)
    session_destroy();
  	unset($_SESSION['student_username']);
 echo '<meta http-equiv ="refresh" content ="0 ;student_login.php">';
   }   
}
}
}
...
{echo '<script language="javascript">alert("Current username is wrong.");</script>';}
  ...
  $sqluser="SELECT password FROM student WHERE password='$password1' ";
  $qresult=mysqli_query($db, $sqluser);
  $count=mysqli_num_rows($qresult);
   if($count == 0)
{echo '<script language="javascript">alert("Current password is wrong.");</script>';}
  ...
  if (empty($student_username1)) { array_push($errors, "Username is required"); }
...
$sql="UPDATE `student` SET password='$password3' WHERE student_username='$student_username1' AND password = '$password1'";	
if($result = mysqli_query($db, $sql))
if(($result) > 0)
