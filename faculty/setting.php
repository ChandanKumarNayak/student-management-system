...
<nav class="navbar navbar-inverse navbar-expand-xl navbar-dark">
	<div class="navbar-header d-flex col">
		...
		<a href="faculty_profile.php" class="nav-link" style="color:white;position:absolute;border:2px solid white;border-radius:10px;top:13px;right:135px;padding:3px;width:50px;font-size:20px">&nbsp;&nbsp;<i class="fa fa-home"></i></a>
	</div>
</nav>  
<?php
...
<div class="w3-row">
  ...
  <center><h3 style="font-size:25px;color:red;width:100%;font-family:Arial"><b>-: USERNAME :-</b></h3></center>
    ...
    <input type="text" class="input" name="faculty_username1" placeholder="Enter current username" required>
    <div class="line-box">
      <div class="line"></div>
    </div>
  </label>
  <label>
	...
	<input type="text" class="input" name="faculty_username2" placeholder="Enter new username" required>
    <div class="line-box">
      <div class="line"></div>
    </div>
  </label>
  <label>
	...
	<input type="password" class="input" name="faculty_username3" placeholder="Confirm new username" required>
    <div class="line-box">
      <div class="line"></div>
    </div>
  </label>
 ...
 if(isset($_POST['change_username']))
{
 ...
 $faculty_username1 = $_POST['faculty_username1'];

  $sqluser="SELECT faculty_username FROM faculty WHERE faculty_username='$faculty_username1' ";
...
{echo '<script language="javascript">alert("Current username is wrong.");</script>';}
 ...
 $faculty_username2 = $_POST['faculty_username2'];

  $sqluser="SELECT faculty_username FROM faculty WHERE faculty_username='$faculty_username2' ";
  $qresult=mysqli_query($db, $sqluser);
  $count=mysqli_num_rows($qresult);
  if($count > 0)
  {
  ...
  {
  $faculty_username3 = $_POST['faculty_username3'];

 
  if (empty($faculty_username1)) { array_push($errors, "Current username is required"); }
  if (empty($faculty_username2)) { array_push($errors, "New username is required"); }
  if (empty($faculty_username3)) { array_push($errors, "Confirm username is required"); }
  if ($faculty_username2 != $faculty_username3) 
   {echo '<script language="javascript">alert("New username & confirm new username did not match.");</script>';}
   else
   if(count($errors) == 0){

$sql="UPDATE `faculty` SET faculty_username='$faculty_username2' WHERE faculty_username='".$_SESSION['faculty_username']."' AND faculty_username = '$faculty_username1'";
...
{echo '<script language="javascript">alert("Current username is wrong.");</script>';}
  ...
  $sqluser="SELECT password FROM faculty WHERE password='$password1' ";
  $qresult=mysqli_query($db, $sqluser);
  $count=mysqli_num_rows($qresult);
   if($count == 0)
{echo '<script language="javascript">alert("Current password is wrong.");</script>';}
  ...
  if (empty($faculty_username1)) { array_push($errors, "Username is required"); }
...
$sql="UPDATE `faculty` SET password='$password3' WHERE faculty_username='$faculty_username1' AND password = '$password1'";	
if($result = mysqli_query($db, $sql))
if(($result) > 0)
