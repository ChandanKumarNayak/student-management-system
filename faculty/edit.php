...
$getdata=("SELECT * FROM `faculty` WHERE id='$eid' AND faculty_username='".$_SESSION['faculty_username']."'");
...
<form method="POST" action="" enctype="multipart/form-data" style="border: 3px solid #001f3f;padding:2px;margin:15px;max-width:600px;min-width:300px">
  ...
  <label>
   &nbsp;&nbsp;Qualification :
    <input type="text" class="input" name="qualification" placeholder="Ex: B.Tech" value="<?php echo $row['qualification']; ?>" required>
    <div class="line-box">
      <div class="line"></div>
    </div>
  </label>
  <label>
   &nbsp;&nbsp;Location :
    <input type="text" class="input" name="location" placeholder="Ex: Talapada,Rahama" value="<?php echo $row['location']; ?>" required>
    <div class="line-box">
      <div class="line"></div>
    </div>
  </label>
...
$sql="UPDATE `faculty` SET name='$name' ,email='$email' ,phone='$phone' ,subject='$subject' ,qualification='$qualification' ,location='$location' WHERE id='$eid' AND faculty_username='".$_SESSION['faculty_username']."'";	
